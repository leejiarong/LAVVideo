...
STDMETHODIMP CDecodeThread::CreateDecoderInternal(const CMediaType *pmt, AVCodecID codec)
{
  ...
  if (!bHWDecBlackList &&  hwAccel != HWAccel_None && !m_bHWDecoderFailed && HWFORMAT_ENABLED && HWRESOLUTION_ENABLED)
  {
	...
	{
	  DWORD dwSupport = m_pLAVVideo->CheckHWAccelSupport(hwAccel);
	  if(dwSupport)
		  m_pDecoder = CreateDecoderQuickSync();
	  else
	  {
		  DbgLog((LOG_TRACE, 10, L"CreateDecoderQuickSync failed! Use DXVA2 native"));
		  dwSupport = m_pLAVVideo->CheckHWAccelSupport(HWAccel_DXVA2Native);
		  if(dwSupport)
			m_pDecoder = CreateDecoderDXVA2Native();
		  else
		  {
			  DbgLog((LOG_TRACE, 10, L"CreateDecoderDXVA2Native failed! Use DXVA2 copy-back"));
